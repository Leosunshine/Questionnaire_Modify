 ...
 /*定义重要的全局变量*/
    ...
    $(function(){
        ...
        $("#Leo_All").click(function(){
            ...
            $.cookie("exam_ans"+<?php echo $number; ?>,ans.join("|"),{experies:7});
            initCookie(questions.length,"exam_ans"+<?php echo $number; ?>);
        });
    ...
    function Leo_initPaperId(){
        var paper_cookie=$.cookie("paper_id"+<?php echo $number; ?>);
        if(!paper_cookie){
            $.cookie("paper_id"+<?php echo $number; ?>,0,{expeires:7});
            paper_id_now=0;
        }else{
    ...
    function changepage(newpage,isCookie){
        ...
        if(isCookie){
            ...
            refreshCookie(Leo_index_now,ans_str,"exam_ans"+<?php echo $number; ?>);
        }
        Leo_index_now=newpage;
        document.getElementById("newdiv_"+newpage).focus();
        initTitle(newpage);
        var inputs=$("input");
        var new_ans=get_ans_array_from_cookie(newpage,"exam_ans"+<?php echo $number; ?>);
        if(new_ans!=-1){
        ...
        $.post('/Examinee/getpaper', {'paper_name':paper_id_name[paper_index]}, function(data) {
            if(data.error){
            	...
            	if(data.no_ques){
                     if(paper_id_now<5){
                        paper_id_now++;
                        $.cookie("paper_id"+<?php echo $number; ?>,paper_id_now,{experies:7});
                        getpaper(paper_id_now);
                        return;
                        }else{
		                     flag = false;
    ...
    function Leo_check(){
    	spinner = new Spinner().spin(target);
        $.post('/Examinee/getExamAnswer',{"answer":$.cookie("exam_ans"+<?php echo $number; ?>),"paper_name":paper_id_name[paper_id_now],"order":ques_order,"time":total_time}, function(data) {
             if(data.error){
             	 if(spinner){ spinner.stop(); }
                 $('.Leo_question_v2').css('width','573px')
                 $('.modal-body').html('');
             	...
             	if(paper_id_now<5){
                    ...
                    $.cookie("paper_id"+<?php echo $number; ?>,paper_id_now,{experies:7});
                    $.cookie("exam_ans"+<?php echo $number; ?>,"",{expires:-1});
                    getpaper(paper_id_now);
                }else{
                	 flag = false;
                	 $('.Leo_question_v2').css('width','573px')
    ...
    function w_last (){
        $.cookie("paper_id"+<?php echo $number; ?>,"",{experies:-1});
        $.cookie("exam_ans"+<?php echo $number; ?>,"",{expeires:-1});
