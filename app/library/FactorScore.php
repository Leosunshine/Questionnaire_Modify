...
class FactorScore{
	...
	protected static function calEPPS(&$resultsets){
		...
		#计算全部因子的得分
		$score_arrays = explode('|', $resultsets->score);
		$score_arrays = array_count_values ($score_arrays);
		unset($score_arrays['']);
		//标准分算法修改--刘衡--0328
		$default_array = array(0=>'agg',1=>'het',2=>'end',3=>'chg',4=>'nur',5=>'aba',6=>'dom',7=>'suc',8=>'int',9=>'aff',10=>'aut',11=>'exh',12=>'ord',13=>'def',14=>'ach');
		$score_array = self::sortByArray($score_arrays,$default_array);	
	...
	public static function sortByArray($sort,$default){
		...
		for ($i=0; $i < $length-1; $i++) { 
			if ($svalue[$i] == $svalue[$i+1]) {
				$temp = $i+1;
				while($temp < $length && $svalue[$temp] == $svalue[$i]){
