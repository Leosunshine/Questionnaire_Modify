...
class EFB extends \Phalcon\Mvc\Controller{
	...
	function divideFactors($factorsArray,&$factorRef,&$factor_ans,$issort,$isspe,$special){
		...
		if ($issort) {
			foreach ($ret as $key => $value) {
				if (!isset($value['name'])) {
					$con = array();
					$con['chs_name'] = $value['chs_name'];
					$con['std_score'] = $value['std_score'];
					$con['rank'] = '';
				}else if($value['name'] != 'con'){
					$new_array[$value['name']] = $value['std_score'];
					$new_name[$value['chs_name']] = $value['name'];
				}else{
					$con['chs_name'] = $value['chs_name'];
					$con['std_score'] = $value['std_score'];
					$con['rank'] = '';
			...
			$default_array = array(0=>'agg',1=>'het',2=>'end',3=>'chg',4=>'nur',5=>'aba',6=>'dom',7=>'suc',8=>'int',9=>'aff',10=>'aut',11=>'exh',12=>'ord',13=>'def',14=>'ach');
			$new_array = self::sortByArray($new_array,$default_array);
			$i = 1;
			foreach ($new_array as $key => $value){
				$new_ans['chs_name'] = array_search($key, $new_name);
				$new_ans['std_score'] = $value;
				$new_ans['rank'] = $i ++;
				$rets[] = $new_ans;
			}
			if (!empty($con)){
    ...
    function checkoutScl(&$information,&$info, &$objActSheet,$scl_paper_id){
        ...
        if(!isset($scl_question_ans) || $scl_question_ans == null ) return;
	...
	//对关联数组排序，相同值时优先级以另一数组为准
	public static function sortByArray($sort,$default){
		arsort($sort);
		...
		for ($i=0; $i < $length-1; $i++) { 
			if ($svalue[$i] == $svalue[$i+1]) {
				$temp = $i+1;
				while($svalue[$temp] == $svalue[$i]){
					if(array_search($skey[$i], $default) > array_search($skey[$temp], $default)){
						$temp_key = $skey[$i];
						$temp_value = $svalue[$i];
						$skey[$i] = $skey[$temp];
						$svalue[$i] = $svalue[$temp];
						$skey[$temp] = $temp_key;
						$svalue[$temp] = $temp_value;
					}
					$temp++;
				}
				$i = $temp - 1;
